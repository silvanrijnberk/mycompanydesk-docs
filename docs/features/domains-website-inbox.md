...
- **Linking** — threads can be linked to a customer, project or invoice for cross-referencing. Threads can also be converted into invoices or quotes; the created document stores a link back to the thread, shown as a **Created from an email** banner with a **view the conversation** link on the document detail page.
