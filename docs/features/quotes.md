...
The [Growth](/features/groei) page groups quotes that need follow-up together with other work that drives the next assignment. It shows how many quotes are waiting for an answer, how many have expired without an answer, and how many expire this week. The same expiring and expired counts also appear in the dashboard **Good news** tab.

Quote requests submitted through your site-builder form also appear on Growth, with the oldest waiting time and a link to **Quotes > Requests**. See [Domains, website and inbox](/features/domains-website-inbox) for setting up the website form.

## Source references

- `apps/web/pages/groei.vue`
- `apps/web/locales/en.json` keys under `groei.quotes` and `dashboard.briefing.goodNews.quotesExpired/quotesExpiring`