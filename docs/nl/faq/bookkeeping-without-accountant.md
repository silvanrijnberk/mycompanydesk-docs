---
title: "Zelf boekhouden zonder boekhouder"
last_verified: 2026-07-21
chatbot:
  triggers:
    - "bookkeeping without accountant"
    - "do my own bookkeeping"
    - "no accountant needed"
    - "zzp without accountant"
    - "zelf boekhouden"
    - "boekhouden zonder boekhouder"
    - "boekhouder nodig"
    - "selbst buchen"
    - "ohne steuerberater"
    - "comptable pas necessaire"
    - "faire sa comptabilite"
  actions:
    - { label: "Open toegangsinstellingen", to: "/nl/settings/team" }
  follow_up:
    - "How do I invite my accountant?"
    - "What does the Free plan include?"
    - "How do VAT calculations work?"
---

# Zelf boekhouden zonder boekhouder

De meeste zzp'ers en eenmanszaken kunnen hun dagelijkse boekhouding prima zelf doen in MyCompanyDesk. De app berekent de btw per factuur en uitgave, scant bonnen met AI en houdt je administratie op één plek.

## Wat MyCompanyDesk voor je doet

- **BTW-berekening**: per factuur en bon wordt het juiste btw-bedrag berekend. Zie [BTW-beheer](/nl/features/vat) voor meer informatie.
- **Bonnen scannen**: maak een foto van je bon en AI leest het bedrag, de btw, de leverancier en de datum uit. Zie [Bonnen scannen](/nl/advanced/receipt-scanning).
- **Overzicht inkomsten en uitgaven**: zie openstaande facturen, betaalde bedragen en uitgaven in één dashboard.
- **BTW-aangifte voorbereiden**: het btw-rapport groepeert je transacties, zodat je de aangifte zelf kunt doen of achteraf aan je boekhouder kunt geven.

## Wanneer je zonder boekhouder kunt

Een eenvoudige eenmanszaak of zzp-onderneming heeft meestal geen boekhouder nodig voor het dagelijks werk. Als je maar een paar facturen per jaar stuurt, je uitgaven bijhoudt en een net btw-overzicht wilt, doe je dit zelf op het Gratis-abonnement.

## Wanneer een boekhouder wel zinvol is

Overweeg een boekhouder bij ingewikkelde situaties, zoals een bv, personeel in dienst of een aangifte waar je niet zeker van bent. Je kunt je boekhouder altijd nog read-only of volledige toegang geven, zonder je wachtwoord te delen. Zie [Boekhouder toegang geven](/nl/faq/accountant-access).

## Kosten

Je kunt starten en blijven op het Gratis-abonnement. Nieuwe werkruimtes krijgen 60 dagen Pro. Betaalde abonnementen staan in [Abonnement & betalingen](/nl/settings/billing).
