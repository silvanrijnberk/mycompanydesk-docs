...
Wandeln Sie den Rest der Arbeit in eine Rechnung um, erhält diese Endrechnung eine negative Position **Verrechnet Anzahlung**, damit der Kunde nur den Restbetrag zahlt. Diese Verrechnung gilt auch, wenn die Arbeit des Angebots über einen Arbeitsbericht fakturiert wird; diese Rechnung kann erst erstellt werden, wenn die Anzahlungsrechnung kein Entwurf mehr ist. Wurde die Arbeit bereits über einen Arbeitsbericht fakturiert, kann dem Angebot keine Anzahlung mehr hinzugefügt werden. Die Anzahlungsrechnung selbst bleibt eine ganz normale Rechnung in Ihren Listen, deren USt. im Zeitraum ihres eigenen Rechnungsdatums fällig wird. Entfernen Sie die Verrechnung, zahlt der Kunde den vollen Betrag zusätzlich zur Anzahlung; Sie können die Anzahlung später erneut verrechnen.
