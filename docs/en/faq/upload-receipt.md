---
title: "Upload receipt"
description: "To upload or replace a receipt on an expense: open an existing expense, or save a new expense first."
last_verified: 2026-08-15
chatbot:
  triggers: ["upload receipt", "attach receipt", "add receipt", "receipt photo", "scan receipt", "expense receipt", "bon uploaden", "bon toevoegen", "beleg hochladen", "telecharger recu"]
  actions:
    - { label: "Open expenses", to: "/expenses" }
  follow_up: ["How do I create an expense?", "How do I categorize an expense?"]
---

To upload or replace a receipt on an expense:
1. Open an existing expense, or save a new expense first.
2. In the quick-view drawer, click the paperclip icon; or on the expense detail page, use the **Upload Receipt** action.
3. Select the image or PDF file you want to attach.
4. Wait for the upload to finish and confirm it appears on the expense.

Note: Receipt files must be 10 MB or smaller. If you scan a file over the limit, the expense is created without the receipt and a warning appears. Upload a smaller file on the expense detail page.

Tip: If you are still on the new-expense form, save the expense first. Once the expense is saved, you can attach or replace its receipt from the quick-view drawer or the detail page.
